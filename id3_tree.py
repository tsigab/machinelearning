...
class id3_tree:
	...
	def calc_information_gain(self, data, classes, feature):
    		""" finding the information gain of every feature in the """

		nData = len(classes)
		nFeatures = len(data[0])
		numClassCounts = dict(Counter(classes))
		
		totalEntropy = 0
		
		for key in numClassCounts.keys():
			
			totalEntropy += self.calc_entropy(
				float(numClassCounts[key]) / nData)
